    ...
    def prepare_inputs(self, x, s, c, uc):
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat"]:
                ...
                tmp = list()
                for uc_tensor, c_tensor in zip(uc[k], c[k]):
                    tmp.append(torch.cat((uc_tensor, c_tensor), 0))
                c_out[k] = tmp
            else:
                assert c[k] == uc[k]
                c_out[k] = c[k]
...
class LinearPredictionGuider(Guider):
    ...
    def prepare_inputs(
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat", "ratio"] + self.additional_cond_keys:
                c_out[k] = torch.cat((uc[k], c[k]), 0)
            elif k == "depth":
                c_out[k] = [torch.cat((uc_tensor, c_tensor)) for uc_tensor, c_tensor in zip(uc[k], c[k])]
            else:
                assert c[k] == uc[k]
                c_out[k] = c[k]
        return torch.cat([x] * 2), torch.cat([s] * 2), c_out
