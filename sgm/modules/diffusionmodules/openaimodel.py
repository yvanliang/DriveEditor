    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(
                ...
                x = layer(x, emb, num_video_frames, image_only_indicator, res_fuse, valid_mask)
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x: th.Tensor, emb: th.Tensor, res_fuse: Optional[th.Tensor] = None, valid_mask=None) -> th.Tensor:
        """
        ...
        if self.use_checkpoint:
            return checkpoint(self._forward, x, emb, res_fuse, valid_mask)
        else:
            return self._forward(x, emb, res_fuse, valid_mask)

    def _forward(self, x: th.Tensor, emb: th.Tensor, res_fuse=None, valid_mask=None) -> th.Tensor:
        if self.updown:
        ...
        if self.use_scale_shift_norm:
            ...
            if res_fuse is not None:
                h = h + self.fuse_layers(res_fuse).type(h.dtype) * valid_mask
            h = self.out_layers(h)
