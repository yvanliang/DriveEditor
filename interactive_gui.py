    ...
    def predict(self, decoding_t, result_draw_box_value, editing):
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                randn_3d = torch.randn(shape_3d, device=self.device)

                additional_model_inputs = {}
                additional_model_inputs["image_only_indicator"] = torch.zeros(2, self.num_frames).to(self.device)
                additional_model_inputs["image_only_indicator_3d"] = torch.zeros(2, self.num_frames_3d).to(self.device)
                additional_model_inputs["num_video_frames"] = self.num_frames
                additional_model_inputs["num_video_frames_3d"] = self.num_frames_3d
                additional_model_inputs["valid_mask"] = torch.concat([valid_mask] * 2).to(self.device)
                additional_model_inputs["mask_fuse"] = torch.concat([mask_fuse_all] * 2).to(self.device)
                additional_model_inputs["obj_ratio"] = torch.concat([obj_ratio] * 2).to(self.device)
                ...
                for d in (c, uc, c_3d, uc_3d):
                    for k, v in d.items():
                        if isinstance(v, torch.Tensor):
                            ...
                            d[k] = [vv.to(self.device) for vv in v]
